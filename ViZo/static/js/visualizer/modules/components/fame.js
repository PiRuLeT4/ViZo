/**
 * ViZzo // fame.js  —  Developers Wall of Fame Generator
 */

export function initWallOfFame() {
  console.log("ViZzo // Inicializando Muro de la Fama de Desarrolladores...");
  const container = document.getElementById("fame-profiles-container");
  if (!container) {
    console.warn("ViZzo // No se encontró el contenedor #fame-profiles-container en la escena.");
    return;
  }

  // Obtener historial de commits desde el estado global asíncrono
  const evolutionData = window.ViZzoState && window.ViZzoState.dataMap && window.ViZzoState.dataMap.evolution_data;
  if (!evolutionData) {
  ...
  console.log("ViZzo // Desarrolladores clasificados:", sortedProfiles);
  ...
  podioOrder.forEach(function (slot) {
    ...
    plate.addEventListener("click", function () {
      ...
      console.log(`ViZzo // Desarrollador seleccionado: ${p.name}`);
    });
